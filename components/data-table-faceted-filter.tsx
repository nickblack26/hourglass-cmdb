							...
							{options.map((option) => {
								...
								return (
									...
									>
										<div
											className={cn(
												'mr-1.5 flex h-4 w-4 items-center justify-center rounded-sm border border-primary',
												isSelected ? 'bg-primary text-primary-foreground' : 'opacity-50 [&_svg]:invisible'
											)}
										>
											<CheckIcon className={cn('h-4 w-4')} />
										</div>
										{option.icon && <option.icon className='mr-1.5 h-4 w-4 text-muted-foreground' />}
